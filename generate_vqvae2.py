...
from Data_Preprocessing.preprocess2 import Preprocessor
from Data_Preprocessing.Utils.utils2 import inv_standardize, expmap_to_euler, resample_motion, split_dataset
...
class Inference:
    ...
    def sample(self, dir_data, name_file, dir_save="./"):
        ...
        try:
            ...
            codebook_path = os.path.join(self.dataset_dir,"codebook_50*192.pth")
        ...
        audio_wav = processed_data["audio_wav"].astype(np.float32)
        audio_wav = rearrange(audio_wav, 'n (block uni_len hop) -> n (block uni_len) hop', block=10, hop=800)
        mo = processed_data['motion'].astype(np.float32)  # [N(1), L, D]
        idx = processed_data['index'].astype(int)  # [N(1), B]
        vid_indices = processed_data["vid_indices"].astype(int) 
        ...
        with torch.no_grad():
            ...
            audio_wav = torch.from_numpy(audio_wav).to(self.device)
            init_lxm_idx = torch.tensor([[init_lxm_idx]]).to(self.device).long()
            vid_indices = torch.from_numpy(vid_indices).to(self.device)
            try:
                lxc = codebook
                ...
                try:
                    ...
                    raise  ValueError
            lxm_idx_pred = self.lxm_intp.generate(aud[:, BL:, :], init_lxm_idx, audio_wav[:,BL:,:])[0].long()
        ...
        with torch.no_grad():
            ...
            ret = self.gen(aud.permute((0, 2, 1)).contiguous(),
                            init_mo.permute((0, 2, 1)).contiguous(),
                            lxm_pred.permute((0, 2, 1)).contiguous(),
                            audio_wav.contiguous(),
                            vid_indices,
                            )
            mo_hat = ret['mo_hat']
            mo_hat = mo_hat.permute((0, 2, 1)).contiguous().cpu().numpy()
    ...
    for f_name in os.listdir(args.data_dir):
        if '.wav' in f_name:
            if f_name[:-4] != args.name_file:
                ...
                warnings.warn('There are other audio files in the data directory. But not remove it.')
                # os.remove(os.path.join(args.data_dir, f_name))   
