    ...
    def train(self) -> None:
        ...
        for epoch in range(1, num_epoch+1):
            ...
            for _, batch in enumerate(self.data_loader_train):
                ...
                logits = self.net(aud[:, self.uniform_len:, :], lxm_idx[:, :-1], wav[:, self.uniform_len:, :])
            ...
            if epoch % valid_num_epoch == 0:
                ...
                with torch.no_grad():
                    for _, batch in enumerate(self.data_loader_val):
                        ...
                        lxm_idx = batch["lexeme_index"].long().to(self.device)  # [N, B].
                        wav = batch["audio_wav"].to(self.device)
                        wav = rearrange(wav, 'n (block uni_len hop) -> n (block uni_len) hop', block=10, hop=800)
                        N = aud.shape[0]
                        ...
                        logits_tf = self.net(aud[:, self.uniform_len:, :], lxm_idx[:, :-1], wav[:, self.uniform_len:, :])
                        lxm_idx_pred_tf = torch.argmax(logits_tf, dim=-1)
                        lxm_idx_pred, logits = self.net.generate(aud[:, self.uniform_len:, :], lxm_idx[:, :1], wav[:, self.uniform_len:, :])
