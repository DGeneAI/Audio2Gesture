...
sys.path.append('/root/project/Audio2Gesture/')
...
class Trainer:
    def __init__(self, config, log_tag) -> None:
        ...
        self.data_loader_train = DataLoader(self.dataset_train, batch_size=self.config['batch_size'], shuffle=True,num_workers=0,pin_memory=False)

        self.dataset_val = TrainingDataset(os.path.join(self.config["dir_data"], "valid.npz"))
        self.data_loader_val = DataLoader(self.dataset_val, batch_size=self.config['batch_size'], shuffle=False,num_workers=0,pin_memory=False)
        ...
        self.motion_encoder=vqvae1d(self.motion_encoder_config['network']['encoder_config'],self.motion_encoder_config['network']['decoder_config'],self.motion_encoder_config['network']['vq_config'])
        self.motion_encoder.load_state_dict(torch.load(path_pretrained_motion_encoder, map_location=torch.device("cpu")))
