...
class Inference:
    ...
    def sample(self, dir_data: str, name_file: str, dir_save: str = "./"):
        ...
        _, _, _,_,_ = split_dataset(dir_audio_feat=os.path.join(dir_save, "Features", "Audio_Feature"),
        ...
        infer_ret= infer_train(batch, self.device, self.net, uniform_len, num_blocks, self.config["network"]["name"])

        motion_gt =infer_ret['mo_gt'].detach().cpu().numpy()  # [N, L, D]
        motion_pred = infer_ret['mo_hat'].detach().cpu().numpy()
